...
#Python Scripting
#September 24, 2015
#Homework 3 - Problem 3 - Iteration 3
...
if hasOr:
	for term in terms:
		goodQuotes = goodQuotes.union(indexed[term])

if hasAnd:
	for term in terms:
		if len(goodQuotes) == 0:
			goodQuotes.update(indexed[term])
		else:
			goodQuotes = goodQuotes.intersection(indexed[term])

for gq in goodQuotes:
	print("Found at ", gq, quotes[gq][:80])
...
# Results
# Description: Quotes broken into dictionaries of key (word): value (indices)

# Query: hundred or thousand
# Time: 10.5 milliseconds

# Query: hundred and thousand
# Time: 7.75 milliseconds